...
- **Recipe version sweep — 80 of 90 recipes bumped to their latest upstream release** (`[package].version` + the version embedded in `[source].url`). `sha256` left empty by design (drafting-grace, [audit F7](docs/audit/2026-04-16.md)); major-version jumps (e.g. `syncthing` 1→2, `libreoffice` 25.2→26.2, `gimp` 3.0→3.2, `ffmpeg` 7→8) may need build-recipe follow-up. Resolution was per-source:
  ...
  - **3 left for manual follow-up** (unresolvable mechanically, source URLs are broken): `stable-diffusion-cpp` (repo has no version tags — the recipe's `v0.2.0` is fictional), `piper-tts` (recipe tag `2024.11.14` doesn't exist; upstream moved to `OHF-Voice/piper1-gpl`), `lmstudio` (closed-source AppImage, dead URL, no public version index — reconsider inclusion).
